...
class SmartDoc(BaseDocument):
    ...
    def get_pk_fields(self):
        ...
        for field in self._fields.values():
            ...
            raise DBError("{} document has no primary key definition".format(self.__class__.__name__))
    
    def find_by_pk(self, pk: str):
        
        pk_parts = pk.split(':')
        pk_fields = [f.split('.', 1)[0] for f in self.get_pk_fields()]
        
        if len(pk_parts) != len(pk_fields):
            raise DBError("{} document has invalid primary key definition: {}, {}"
                          .format(self.__class__.__name__, pk_fields, pk_parts))
        
        return self.find_one(**dict(zip(pk_fields, pk_parts)))
    ...
    def load(self, src_path=OnBoard.META_DIR, ignore=False):
        ...
        if not hasattr(self, '_FILE_NAME'):
            ...
            raise NotImplementedError(
                ...
                """because a _FILE_NAME hasn't been defined for its type""".format(self.__class__.__name__)
            )
